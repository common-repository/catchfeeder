<?php
/**
 * Plugin Name: catchfeeder
 * Description: Connect your WooCommerce to Catch
 * Version: 3.13
 * Author: Catch
...
if ((get_option('active_plugins') && in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) ||
    ...
    if (!class_exists('CatchFeederAdmin')) {

        class CatchFeederAdmin {
            const baseUrl = 'https://catchfeeder.omnivore.com.au/wooCommerce/connect';
            const flavour = 'catchfeeder';
            const flavourLabel = 'CatchFeeder';
            const keyName = 'CityBeach CatchFeeder';
    ...
    if (!class_exists('CatchFeeder')) {

        class CatchFeeder {
            protected $tag = 'catchfeeder';
            protected $name = 'CatchFeeder';
